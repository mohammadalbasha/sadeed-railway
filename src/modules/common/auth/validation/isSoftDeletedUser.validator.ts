...
interface IsSoftDeletedUserValidationArguments extends ValidationArguments {
  // constraints: [
...
@ValidatorConstraint({ name: 'isSoftDeletedUser', async: true })
@Injectable()
export class IsSoftDeletedUser implements ValidatorConstraintInterface {
  constructor(@InjectConnection() private connection: Connection) {
  }

  public async validate(value: string , args: IsSoftDeletedUserValidationArguments) {
 ...
 if (!result) return false
    ...
    // }
    return `user must be soft deleted`;
    // if (args.constraints === undefined) {
